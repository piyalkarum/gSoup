...
List calculate_pi(CharacterMatrix alignment) {
  ...
  std::set<std::string> valid_bases = {"A", "C", "G", "T"};  // Only valid nucleotide bases
  ...
  for (int j = 0; j < n; j++) {
    for (int k = j + 1; k < n; k++) {
      ...
      for (int i = 0; i < len; i += 3) {
        ...
        // Skip codons with ambiguous nucleotides, gaps, or stop codons
        if (codon1.find_first_not_of("ACGT") != std::string::npos || codon2.find_first_not_of("ACGT") != std::string::npos) {
          continue;
        }
        ...
        // Count differences, avoiding double-counting shared changes
        if (aa1 != aa2) {
          seq_nonsyn_diffs += 1;
